...
| **커뮤니티** | 자료 공유 피드 · 주제 게시판 · 댓글 | `010` | `community/`, `lib/communityApi.ts` |
...
**데스크톱 사이드바:** 대시보드 · 도구 · 개별 학습 자료 · 커뮤니티 · 스토어 · 설정  
(+ 관리자: 주문 관리 · 콘텐츠 · 품질)

**모바일 하단 탭:** 홈 · 도구 · 자료 · 커뮤니티 · 스토어 · 설정

| 메뉴 | 내부 구성 |
|------|-----------|
| **도구** | 탭 **수업 도구** \| **업무 도구** |
| **개별 학습 자료** | 탭 **새 자료** \| **내 자료** (라이브러리 통합) |
| **커뮤니티** | 탭 **자료 공유** \| **질문·답변** \| **수업 팁** (+ 댓글) |
...
- **학생 정보:** 등록·수정·삭제 · **순서 바꾸기** 토글(▲▼) · 사진 (`011`)
- **학생 사진 (선택):** 상반신 권장 · 목록 썸네일 · Storage `student-photos` (`008`) · 화이트보드 스탬프 연동

### 개별 학습 자료
- **새 자료:** 카탈로그에서 유형 선택 → 생성 (검색·카테고리 칩)
- **즐겨찾기:** 별표로 상단 고정 (브라우저 저장)
...
### 도구
- 수업 도구 목록 **화이트보드가 맨 앞** · 별표 즐겨찾기로 상단 고정
- **수업 도구:** 화이트보드 · 타이머(BGM) · 스톱워치 · 아날로그 시계 · 랜덤 뽑기(학생 사진) · 사다리 · 주사위 · 가위바위보 · 계산기 · 박수·알림 · 모둠 섞기 · 출석·호명 · 점수판
- **화이트보드:** 펜·마커·형광펜·부분 지우개(크기 조절) · 도형 · 텍스트 · 이모지/학생 얼굴 스탬프 · 실행 취소/다시 실행 · 격자·줄노트 배경 · 레이저·스포트라이트 · 여러 페이지 · PNG 저장·복사
...
### 커뮤니티 (교사 소통 · 자료 공유)
- **자료 공유:** 내 자료(스토리텔링·그림일기)를 골라 스냅샷과 함께 공유 · 활용 후기 작성
- **질문·답변 / 수업 팁:** 주제별 게시판
- **댓글:** 게시글별 대화로 소속감·피드백
- 데모: 샘플 글·댓글 localStorage · 실계정: 마이그레이션 `010_community.sql`
...
│   │   └── adminContentApi.ts
│   ├── types/                   # store · community · navigation · adminContent
│   └── contexts/                # Auth (isDemo, isAdmin)
└── supabase/migrations/         # 001–003 학습 · 004–007 스토어 · 008 사진 · 009 품질 · 010 커뮤니티
```
...
| 9 | `010_community.sql` | 커뮤니티 게시글·댓글 · RLS |
| 10 | `011_student_sort_order.sql` | 학생 목록 ▲▼ 정렬 순서 |
...
- **개별 학습 자료** 화면에서 생성·내 자료 통합 관리 · 자료 유형 즐겨찾기
- PDF 저장 · 미리보기 · 검색
- 학생 사진(선택) — 랜덤 뽑기 · **화이트보드 원형 얼굴 스탬프**와 연동
- 학생 목록 **순서 바꾸기** 모드(`011`) — 앱 전역 학생 순서에 반영
- Supabase Auth + RLS 계정별 격리
- 데모 모드로 가입 없이 UI·스토어·커뮤니티 체험 (AI 생성은 가입 유도)

### 수업 · 업무 도구
- 교실에서 바로 쓰는 화이트보드·타이머·시계·뽑기·점수판 등
- **화이트보드**를 PLMA 차별 도구로 강화(학생 스탬프·다기능 그림판)
- 업무용 메모·할 일·스케줄·리마인더·학생 간단 기록 (브라우저 저장)
- 수업·업무 도구 카탈로그 **즐겨찾기(별)** 상단 고정

### 교사 커뮤니티
- **자료 공유**가 핵심 + **질문·답변 / 수업 팁** 게시판 + 댓글
- 메뉴: 커뮤니티(학습 자료 다음) · 스토어 · 설정
- DB: `010_community.sql`
...
- 자료·도구 카드 **설명 한 줄 truncate** (`line-clamp-1`)

### 인증 · UI · 내비
- Supabase Auth 회원가입/로그인, RLS 재실행 안전 (`DROP POLICY IF EXISTS`)
- 데모: 관리자 메뉴 숨김 · AI 생성 시 회원가입 유도 (`exitDemoForSignUp`)
- 사이드바: 대시보드 · 도구 · 개별 학습 자료 · **커뮤니티** · 스토어 · 설정
- **모바일 하단:** 홈 · 도구 · 자료 · 커뮤니티 · 스토어 · 설정 (더보기 시트 제거)
...
- **학생 순서:** `sort_order` (`011`) · **순서 바꾸기** 토글 시에만 ▲▼ 표시 · 모드 중 수정/삭제 흐림 처리

### 도구 · 화이트보드
- **아날로그 시계:** 분침 드래그 시 각도 델타로 시 넘김
- **즐겨찾기 별:** 테두리 유지 + 노란색 채움 · 수업/업무/학습 자료 공통
- **화이트보드:** 전체 지우기만 있던 지우개를 **부분 지우개**로 개선 · 필기 도구 복수화
- 실행 취소/다시 실행 · 도형 · 배경 · 텍스트 · 스탬프 · 레이저/스포트 · 멀티 페이지 · PNG 저장/복사
- **학생 얼굴 원형 스탬프** (사진 없으면 이니셜) · PNG 공유 시 얼굴 포함 안내
...
### 커뮤니티
- 자료 공유 피드 + 주제 게시판 + 댓글 MVP (`010`)
- 데모 시드 글 · 실계정은 마이그레이션 필요
...
| 커뮤니티 | `src/components/community/CommunityPage.tsx`, `src/lib/communityApi.ts` |
| 화이트보드 | `src/components/tools/WhiteboardTool.tsx`, `whiteboardStamp.ts` |
| 즐겨찾기 | `src/hooks/useToolFavorites.ts`, `FavoriteStarButton.tsx` |
| 학생 순서 | `011_student_sort_order.sql`, `useStudents.moveStudent` |
| 내비 페이지 id | `src/types/navigation.ts` |
| RLS·스키마 | `supabase/migrations/` (`001`–`011`) |
...
4. Supabase에서 마이그레이션 `001`–`011` 적용 (스토어 `004`–`007`, 학생 사진 `008`, 콘텐츠 품질 `009`, 커뮤니티 `010`, 학생 정렬 `011`)
