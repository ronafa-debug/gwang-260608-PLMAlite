...
const tabs: Array<{ id: SettingsTab; label: string }> = [
  { id: 'teacher', label: '교사 정보 관리' },
  { id: 'students', label: '학생 정보 관리' },
]
...
export function SettingsPage({
  ...
  return (
    <div className={cn('space-y-6', activeTab === 'teacher' && 'mx-auto max-w-2xl')}>
      <div>
        <h1 className="text-2xl font-bold text-foreground">설정</h1>
        <p className="mt-1 text-muted-foreground">교사·학생 정보를 관리합니다.</p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent className="space-y-4">
              {isDemo ? (
                <DemoNotice>
                  데모에서는 배송 정보가 이 기기에만 저장됩니다. 실제 계정 연동 시 profiles
                  테이블에 저장되며 주문 시 기본값으로 쓰입니다.
                </DemoNotice>
              ) : null}
              {shippingLoading ? (
